...
//主题名字，可在控制台新建
String TOPIC = "SN001001003"; //用于传输温湿度的主题
//设备SN码
String deviceSN = "SN001001003";
...
2-----> GPIO2 本地开关
***/
const int SWITCH_Pin = 0;  //ESP-01s 连接开关的引脚 GPIO0
  ...
  else
  {
    ...
    if((TcpClient_Buff.length() >= 1) && (millis() - TcpClient_preTick>=200))
      { //当订阅后，其它设备发布信息将会自动接收
        ...
        int lastIndex = TcpClient_Buff.lastIndexOf("&msg=");
        if(lastIndex != -1){
          String msgValue = TcpClient_Buff.substring(lastIndex + 5);
          msgValue.trim();
...
void setup() {
  ...
  //连续复位3次，恢复出厂设置
  if(resetTime >= 3){
    for(int i = 0; i < EEPROM_FULL_ADDR; i++){
        EEPROM.write(i,0);
    }
  ...
  g_wifiSSID = readStringFromEEPROM(EEPROM_SSID_ADDR);
  g_wifiSSID.trim();
  g_wifiPassword = readStringFromEEPROM(EEPROM_PASSWORD_ADDR);
  g_wifiPassword.trim();
  EEPROM.end();
